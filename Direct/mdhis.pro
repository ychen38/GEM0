...
nn = 1
nskip=100L
if(nskip GT 1) then begin
for i = 1,nn do begin
   for k=nskip,n-nskip do begin
      sum = 0.0
      for j=1,nskip do begin
         sum = sum+a1(k+j-nskip/2)
      endfor
      sum = sum/nskip
      a1a(k) = sum
;      dum(k) = (0.5*a1(k-1)+a1(k)+0.5*a1(k+1))/2.0
   endfor
;   a1 = dum
endfor
endif
for k=0L,n-1 do begin
   print, time(k)/omega,a1a(k)
endfor
...
xdum=[-1,-2]
ydum=[1,1]
plot, xdum, ydum, title='', xtitle='!4x!l!3A!Nt', ytitle='rms(!4u!3)',linestyle=0, xrange=[0,time(n-1)],yrange=[1e-5, 2e-1],/ylog
