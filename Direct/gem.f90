...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      REAL(8) :: wx0,wx1,wy0,wy1,wz0,wz1,dum,vxdum,dum1,bstar
      ...
      REAL(8) :: grp,gxdgyp,rhox(4),rhoy(4),psp,pzp,vncp,vparspp,psip2p,bdcrvbp,curvbzp,dipdrp
      integer :: mynopi
...
!         if(x3(ns,m)>lx .or. x3(ns,m)<0.)w3(ns,m) = 0.
 ...
 333     continue
         ...
         y3(ns,m)=dmod(y3(ns,m)-laps*2*pi*qr*lr0/q0*dsign(1.d0,q0)+8000.*ly,ly)
         ...
         x3(ns,m)=dmod(x3(ns,m)+800.*lx,lx)         
...
!-----------------------------------------------------------------------
      ...
      REAL(8) :: myke,mypfl_es(1:nsubd),mypfl_em(1:nsubd),myavewi
      REAL(8) :: myefl_es(1:nsubd),myefl_em(1:nsubd),mynos
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      implicit none
      INTEGER :: ns
      REAL(8) :: b,gam0,gam1,delyz,th,bf,dum,r,qr,shat
      REAL(8),DIMENSION(1:5) :: b1,b2,ter
      REAL(8) :: kx1,kx2,ky
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      implicit none
      integer :: i,j,k,l,m,n
      real(8) :: pf,efe,efi,pfi,efc,pfc,efb,pfb,pflxgb,eflxgb,x
      real(8) :: pf_em,efe_em,efi_em,pfi_em,efc_em,pfc_em,efb_em,pfb_em
      real(8) :: tdum,v(0:imx)
      ...
      if(myid.eq.master)then
         ...
         write(*,10)i,rmsphi(n),rmsapa(n),pfc,efc,avewi(1,n),avewi(2,n),avewe(n),tot_field_e,tot_joule,tot_joule1
 ...
 15      format(1x,i6,8(2x,e10.3))
         ...
         write(9,10)i,rmsphi(n),rmsapa(n),pfc,efc,avewi(1,n),avewi(2,n),avewe(n),tot_field_e,tot_joule,tot_joule1

         write(11,12)i,pf/pflxgb,pfi/pflxgb,pfc/pflxgb,efe/eflxgb,efi/eflxgb,efc/eflxgb,pf_em/pflxgb,pfi_em/pflxgb,pfc_em/pflxgb,efe_em/eflxgb,efi_em/eflxgb,efc_em/eflxgb
      ...
      if(gclr==kmx/2 .and. tclr==0)then
         ...
         write(23,16)tdum,mdhis(0),mdhis(1),mdhis(2),mdhis(3),mdhis(4),mdhisa(0),mdhisa(1),mdhisa(2),mdhisa(3),mdhisa(4)
         write(24,16)tdum,mdhisb(0),mdhisb(1),mdhisc(0),mdhisc(1),mdhisd(0),mdhisd(1)
         write(25,17)tdum,(v(i),i = 0,imx-1)

         do  i = 0,6
            write(22,14)tdum,i,real(phihis(i,0)),(real(phihis(i,j)), aimag(phihis(i,j)), j = 1,jcnt-2,2)
            write(22,14)tdum,i,real(aparhis(i,0)),(real(aparhis(i,j)), aimag(aparhis(i,j)), j = 1,jcnt-2,2)
         end do
         close(22)
         close(23)
         close(24)
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      implicit none
      integer :: i,j,n
      real(8), dimension(1:nsubd) :: roa
      real(8) :: coefu

      coefu = xu**2*frequ
      do j = 1,nsubd
         i = int(nr*(2*j-1)/(2*nsubd))
         chii(j,n) = efl_es(1,j,n)/(xn0s(1,i)*cn0s(1)*t0s(1,i)*capts(1,i))
         chie(j,n) = efle_es(j,n)/(xn0e(i)*cn0e*t0e(i)*capte(i))
         ddi(j,n) = pfl_es(1,j,n)/(xn0s(1,i)*cn0s(1)*capns(1,i))
         achii(j) = achii(j)*.95d0+chii(j,n)*.05d0
         achie(j) = achie(j)*.95d0+chie(j,n)*.05d0
         addi(j) = addi(j)*.95d0+ddi(j,n)*.05d0
         roa(j) = (rin+dr*i)/a
      enddo
      
      if(myid.eq.master)then
         open(911, file='d_chi', status='unknown')
         write(911,11) int(tcurr-dt)
         write(911,12) '        r/a       chi_i       chi_e       D_i'
         do j = 1,nsubd
            write(911,10) roa(j), achii(j)*coefu,achie(j)*coefu,addi(j)*coefu
         enddo
         close(911)
      end if
 10      format(4(2x,e12.4))
 11      format('time = ', I6)
 12      format(A)
      end 
...
!         go to 333
 ...
 333     continue
         ...
         wx1 = 1.-wx0
         qr = wx0*sf(i)+wx1*sf(i+1)
         y3e(m)=dmod(y3e(m)-laps*2*pi*qr*lr0/q0*dsign(1.d0,q0)+8000.*ly,ly)
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      REAL(8) :: xt,xs,yt,zt,xdot,ydot,zdot,pzdot,edot,pzd0,pzd1,vp0
      REAL(8) :: wx0,wx1,wy0,wy1,wz0,wz1,w3old
      REAL(8) :: myke,mypfl_es(1:nsubd),mypfl_em(1:nsubd),myptrp
      REAL(8) :: myefl_es(1:nsubd),myefl_em(1:nsubd),mynos,myavewe
...
!      upa0(:,:,:) = apar(ip,:,:,:)
      do i = 0,im
         do j = 0,jm
            rbfs(i,j)=(jion(i,j,0)*ision-q(1)*q(1)/mims(1)*gn0s(1,i)*cn0s(1)*apar(i,j,0)*bdgrzn(i,0)/ntube*isiap-(upazd(i,j,0)+amie*upa0(i,j,0)))*jac(i,0)
         end do   
      end do
      ...
      do i = 0,im
         do j = 0,jm
            lbfs(i,j)=(jion(i,j,1)*ision-q(1)*q(1)/mims(1)*gn0s(1,i)*cn0s(1)*apar(i,j,1)*bdgrzn(i,1)/ntube*isiap-(upazd(i,j,1)+amie*upa0(i,j,1)))*jac(i,1)
         end do   
      end do
      ...
      do i = 0,im-1
         do j = 0,jm-1
       	...
       	+weightpn(i)*lbfr(i,jpn(i,j))
            drhodt(i,j,0) = -((jion(i,j,1)*ision-q(1)*q(1)/mims(1)*gn0s(1,i)*cn0s(1)*apar(i,j,1)*bdgrzn(i,1)/ntube*isiap-upazd(i,j,1) &
            ...
            dum = weightm(i)*rbfr(i,jmi(i,j)) &
                +weightmn(i)*rbfr(i,jmn(i,j))
            drhodt(i,j,1) = -(dum-(jion(i,j,0)*ision-q(1)*q(1)/mims(1)*gn0s(1,i)*cn0s(1)*apar(i,j,0)*bdgrzn(i,0)/ntube*isiap-upazd(i,j,0)  &
...
!    right now only ion quantitities are calculated...
      ...
      REAL(8) :: xnp,ter,dbdrp,dbdtp,grcgtp,bfldp,fp,radiusp,dydrp,qhatp,psipp,jfnp,grdgtp
      REAL(8) :: grp,gxdgyp,psp,pzp,psip2p,bdcrvbp,curvbzp,dipdrp,bstar
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      REAL(8) :: wx0,wx1,wy0,wy1,wz0,wz1,vte
      REAL(8) :: wght,r,bfldp,ter,b,vfac,th
      REAL(8) :: xt,yt,zt
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do j = 0,jmx-1
         do i = 0,imx-1
            ...
            if(onemd==1.and.m1.ne.mlk)filterx(i,j) = 0.d0
!            if(m1>1)filterx(i,j) = 0.d0
            if(j==0.and.i<=nzcrt)filterx(i,j) = 0.d0
            if(m1>0.and.m1<nlow)filterx(i,j) = 0.d0!2.d0/imx*float(m1*m1)/(nlow**2)
            if(j==0 .and. ineq0==0)filterx(i,j) = 0.d0
         end do
      end do
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do 51 j = 0,im-1
         ...
         wx1 = 1.-wx0
         qr = wx0*sf(i)+wx1*sf(i+1)
         dely(j) = dmod(-pi2*lr0/q0*qr*dsign(1.d0,q0)+8000.*ly,ly)*tor
 51   continue
      do 52 j = 0,jm-1
         if(j.ge.(jm/2+1)) then
...
!    LINEAR: perturb w(m) to get linear growth...
         w2e(m)=0. !2.*amp*(revers(MyId*cnt+m,13)-0.5) !(ran2(iseed) - 0.5 )
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real(8) :: bfldp,ter
      REAL(8) :: wx0,wx1,wz0,wz1
      REAL(8) :: r,qr,th,cost,sint,b,vfac,js,jv,dum,dum1,pidum,vel
      REAL(8) :: xt,yt,zt,emac=10.,dvfac,dlamb,lamb,dely,g,myavewe,avwe,avwen
      integer :: isign,il,ie,bfcnt,mynobge
      real(8) :: w(0:1,0:1,0:1,0:1)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      REAL(8) :: wx0,wx1,wz0,wz1
      REAL(8) :: r,qr,th,cost,sint,b,vfac,js,jv,dum,dum1,pidum,vel
      REAL(8) :: xt,yt,zt,emac=10.,dvfac,dlamb,lamb,dely,g,myavewe,avwi,avwen
      integer :: isign,il,ie,bfcnt,mynobge
      real(8) :: w(0:1,0:1,0:1,0:1)
      real(8) :: mytotw(0:imx-1,0:jmx-1,0:negrd-1,0:nlgrd-1), &
                 totw(0:imx-1,0:jmx-1,0:negrd-1,0:nlgrd-1), &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real(8) :: rbfr(0:imx,0:jmx)
      real(8) :: rbfs(0:imx,0:jmx)
      REAL(8),DIMENSION(1:5) :: b,ter,gam0,gam1
      REAL(8) :: b2,delyz,th,bf,gamphi
      REAL(8) :: kx,ky,wx0,wz0,wx1,wz1
      REAL(8) :: dbdrp,dbdtp,grcgtp,bfldp,fp,radiusp,dydrp,qhatp,psipp,jfnp
      REAL(8) :: r,grp,gxdgyp,grdgtp,gthp
      real(8),dimension(:),allocatable :: akx,aky
      ...
      real(8) :: myden(0:imx,0:jmx,0:1),rho1(0:imx,0:jmx,0:1)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      REAL(8) :: kx,ky,wx0,wz0,wx1,wz1,ter,terc
      REAL(8) :: dbdrp,dbdtp,grcgtp,bfldp,fp,radiusp,dydrp,qhatp,psipp,jfnp
      REAL(8) :: r,grp,gxdgyp,grdgtp,gthp
      REAL(8),DIMENSION(:),ALLOCATABLE :: akx,aky
      REAL(8),DIMENSION(:,:,:), ALLOCATABLE :: formapa
      REAL(8) :: sgnx,sgny,sz,xfrac=1.0
      INTEGER :: i,j,k,i1,k1,l,m,n,ifirs,ip,nstep
      INTEGER :: l1,m1,myk
      COMPLEX(8) :: temp3d(0:imx-1,0:jmx-1,0:1)
      REAL(8) :: myrmsapa
      real(8) :: myden(0:imx,0:jmx,0:1),jtmp(0:imx,0:jmx,0:1)
...
!!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real(8) :: domega,om,dum1,dum2,dum3,dum4,dum5,x,gam,peak(0:6,1:5)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      implicit none
      integer :: i,ip,k,m,n,ncol,icol
      real(8) :: edum,vdum,dum,dum1,ptch,vti,r,qr,th,cost,b
      real(8) :: h_x,h_coll,x,eps,dtcol,uold,hee,nue,ter
      real(8) :: wx0,wx1,wz0,wz1
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      INTEGER :: l1,m1,myk,myj,ix,ikx,ierror
      real(8) :: u(0:imx,0:jmx,0:1)
      COMPLEX(8) :: temp3d(0:imx-1,0:jmx-1,0:1),v(0:imx-1),u1(0:imx-1)
      real(8) :: filterx(0:imx-1,0:jmx-1)
      real(8) :: kx,ky,sgnx,sgny,dum,dum1,b2,myuav(0:imx-1)
      real(8) :: myjaca(0:imx-1),jaca(0:imx-1),uav(0:imx-1)
      ...
      do j = 0,jmx-1
         do i = 0,imx-1
            ...
            ky=sgny*2.d0*pi*float(m1)/ly
            kx=sgnx*2.d0*pi*float(l1)/lx
            b2 = xshape**2*kx**2+yshape**2*ky**2

            filterx(i,j) = 1.d0/(imx*jmx)

            if(abs(ky)>kycut)filterx(i,j) = 0.d0
            if(abs(kx)>kxcut)filterx(i,j) = 0.d0
            if(onemd==1.and.m1.ne.mlk)filterx(i,j) = 0.d0
!            if(j==0.and.i<=nzcrt)filterx(i,j) = 0.d0
!            if(m1>0.and.m1<nlow)filterx(i,j) = 0.d0 
         end do
      end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real(8) :: u(0:imx,0:jmx,0:1),v(0:imx)
      real(8) :: myjac(0:imx),mydbr(0:imx),totjac(0:imx)
      real :: dum,dum1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      implicit none
      INTEGER :: i,j,k,k1,l,m,n
      real(8) :: v(0:imx),upaneq0(0:imx,0:jmx,0:1),upaneq8(0:imx,0:jmx,0:1)
      real(8) :: exneq1(0:imx,0:jmx,0:1),eyneq1(0:imx,0:jmx,0:1)
      real(8) :: exneq2(0:imx,0:jmx,0:1),eyneq2(0:imx,0:jmx,0:1)
      real(8) :: exneq3(0:imx,0:jmx,0:1),eyneq3(0:imx,0:jmx,0:1)
      real(8) :: uptot(0:imx,0:jmx,0:1),dbyneq0(0:imx,0:jmx,0:1),dbxneq0(0:imx,0:jmx,0:1)
      real(8) :: myjac(0:imx),mydbr(0:imx),totjac(0:imx)
      real :: dum,dum1,dum2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real(8) :: u(0:imx,0:jmx,0:1),v(0:imx,0:jmx,0:1)
      real :: dum,dum1
