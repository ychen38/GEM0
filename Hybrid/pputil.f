...
!
    DO i=1,nvp
       ...
       CALL MPI_SENDRECV(s_buf, nxp*nyp, MPI_COMPLEX16, idsr(i), i,&
            & r_buf, nxp*nyp, MPI_COMPLEX16, idsr(i), i,&
...
!
    DO iter=1,nvp
       ...
       CALL MPI_SENDRECV(s_buf, nxp*ny*nzp, MPI_COMPLEX16, idsr(iter), iter,&
            & r_buf, nxp*ny*nzp, MPI_COMPLEX16, idsr(iter), iter,&
            & MPI_COMM_WORLD, status, ierr)
...
!
!    call ccfft(0, nx, 1.0, dummy, dummy, tablex, workx, 0)
!    call ccfft(0, ny, 1.0, dummy, dummy, tabley, worky, 0)
!
    do j=1,nyp
!       call ccfft(isign, nx, 1.0, f(1,j), f(1,j), tablex, workx, 0)
    end do
    CALL pptransp(f, g)
    do i=1,nxp
!       call ccfft(isign, ny, 1.0, g(1,i), g(1,i), tabley, worky, 0)
    end do
...
!
    DO j=1,nyp
       DO k=1,nz
!          CALL ccfft(isign, nx, 1.0, f(1,k,j), f(1,k,j), tablex, workx, 0)
       END DO
    END DO
    CALL pptransp(f, g)
    DO i=1,nxp
       DO k=1,nz
!          CALL ccfft(isign, ny, 1.0, g(1,k,i), g(1,k,i), tabley, worky, 0)
       END DO
    END DO
